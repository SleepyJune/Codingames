...
class BFS
{
    public static Troop UseBFS(Troop troop, Factory end)
    {
        ...
        foreach (var factory in Factory.factories.Values) //initialize
        {
...
class Factory : Entity
{
    ...
    public static int neutralTotalProduction = 0;
    ...
    public static bool allyProductionMaxed = false;
    ...
    public static void CleanUp()
    {
        foreach (var factory in factories.Values)
        {
            ...
            factory.averageEnemyDistance = 0;
            factory.enemyFactoryCount = 0;
        }

        Factory.enemyTotalProduction = 0;
        Factory.allyTotalProduction = 0;
        Factory.neutralTotalProduction = 0;
        ...
        Factory.allyProductionMaxed = true;
    }
    ...
    public static void ProcessNeighbourCounts()
    {
        foreach (var factory in factories.Values)
        {
            ...
            foreach (var pair in factory.links)
            {
                ...
                if (factory.isAlly)
                {
                    ...
                    else if (neighbour.team == Team.Enemy)
                    {
                        ...
                        factory.averageEnemyDistance += (int)distance;
                        factory.enemyFactoryCount += 1;
                    }
            ...
            factory.averageEnemyDistance = factory.enemyFactoryCount == 0 ? int.MaxValue :
                factory.averageEnemyDistance / factory.enemyFactoryCount;
        }
    }
    ...
    public void ProcessMessage(EntityMessage message)
    {
        ...
        {
            SetTeam(Team.Neutral);
            Factory.neutralTotalProduction += production;
        }
...
class Game
{
    ...
    public static void EnlistTroopsForMission(Mission mission)
    {
        ...
        {
            if (ally.isAlly && ally.count > 0 && mission.factory != ally)
            {
                ...
                if (ally.oldProduction < 3 && ally.averageEnemyDistance > 7.5
                    && Factory.neutralTotalProduction == 0
                    && Factory.allyTotalProduction < Factory.enemyTotalProduction)
                {
                    //robotsAvailable -= 10;
                }
    ...
    public static void SolveCaptureMission(Mission mission)
    {
        ...
        if (Factory.allyProductionMaxed)
        {
            //do it anyways for teh luls
            foreach (Troop troop in newEnlisted)
            {
                Action newAction = new Action(MoveType.Move, troop.start, troop.end, troop.count);
                AddAction(newAction, troop);
    ...
    public static void SolveCaptureMission2(Mission mission)
    {
        foreach (Troop troop in mission.acceptedMission)
        {
            Action newAction = new Action(MoveType.Move, troop.start, troop.end, troop.count);
            AddAction(newAction, troop);
        }
    ...
    public static void MakeMove()
    {
        ...
        missions.AddRange(CaptureFactories().OrderBy(m=>m.type));
    ...
    public static List<Mission> SendBomb()
    {
        ...
        foreach (var bomb in Bomb.bombs.Values)
        {
            if (bomb.isAlly)
            {
                ...
                {
                    if (bomb.end == troop.end
                        && (bomb.turns + 1 == troop.turns
                        || bomb.turns + 2 == troop.turns
                        || bomb.turns + 3 == troop.turns))
                    {
                        troopSent = true;
                        break;
                    }
                ...
                if (troopSent == false)
                {
                    Mission bombMission = new Mission(MissionType.Bomb, bomb.end, bomb.turns);
