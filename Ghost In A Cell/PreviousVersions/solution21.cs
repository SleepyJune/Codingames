...
class MissionReward
{
    ...
    public MissionReward(int gained, int lost)
    {
    ...
    public static MissionReward CalculateReward(Mission mission, int endTime)
    {
        int gained = 0;
        int lost = 0;

        int rewardTime = endTime - mission.missionEndTime;
        ...
        if (mission.type == MissionType.Inc)
        {
            gained -= 10; //lost 10 troops to production increase;
            gained += productionReward;
        }

        return new MissionReward(gained, lost);
    }
}

class MissionComparer
{
...
class MissionPlanner
{
    ...
    public void MakeMockTroop(Factory ally, int count)
    {
        ...
        if (mockTroop.end.team == Team.Enemy && mockTroop.end != mission.factory)
        {
            return; //troop goes through enemy land
        }
...
class Strategy
{
    ...
    public static void ExecuteMission()
    {
        ...
        {
            if (mission.successRating != MissionSuccessRating.Impossible)
            {
