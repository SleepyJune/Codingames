...
class BFS
{
    ...
    public static Troop UseBFS(Troop troop, Factory end)
    {
        ...
        {
            if (directDistance * 1.5 <= totalDistance
                || directDistance <= 5)
            {
...
class Mission : IComparable<Mission>
{
    ...
    public void CalculateReward()
    {
        ...
        {
            rewardTime = Math.Max(0, mission.factory.nearestEnemyDistance - turns + 1);
        }        
...
class BombMissionSolver : MissionSolver
{
    ...
    public void Solve()
    {
        ...
        if (lastState.team == Team.Enemy && factory.production == 3
            && !Bomb.bombs.Values.Any(b => b.isAlly && b.end == mission.factory) //no bombs going there
            && mission.acceptedMission.Count > 0)
        {
            Troop closestTroop = mission.acceptedMission.First();
            int distance = closestTroop.start.links[factory];
                        
            if (true)//closestTroop.end == factory)
            {
                Bomb bomb = new Bomb
                {
                    isAlly = true,
...
class CaptureMissionSolver : MissionSolver
{
    ...
    public void Solve()
    {
        ...
        foreach (var mockTestTroop in mission.acceptedMission)
        {
            ...
            {
                if (mockTestTroop.count - armyAvailable + 1 <= 0)
                {
...
class MissionReward
{
    ...
    public double combinedRewardWeighted;
    ...
    public static bool GetCaptureProbability(Mission mission, int defendingArmy, int turns)
    {
        ...
        foreach (var pair in mission.factory.links)
        {
            ...
            if (neighbour.isEnemy && distance <= turns)
            {
                ...
                {
                    maxEnemyTroopCount += state.count;
                }
            }
        }
...
class Strategy
{
    ...
    public static bool CheckLaterRewards(int index)
    {
        ...
        {
            rewards += Math.Max(0, missions[i].reward.combinedRewardWeighted);
        }

        double currentReward = missions[index].reward.combinedRewardWeighted;
    ...
    public static bool ExecuteMissions()
    {
        ...
        for (int i = 0; i < missions.Count; i++)
        {
            ...
            //Console.Error.WriteLine(mission.factory.id + ": " + mission.reward.allyTroopGained);

            if (mission.isPossible() && ClearedPrereq(mission))
            {
                Console.Error.WriteLine(
                    "Execute " + mission.type.ToString() + " " + mission.factory.id);
