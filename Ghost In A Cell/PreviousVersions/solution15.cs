...
class BFS
{
    public static Factory UseBFS(Factory start, Factory end)
    {
    ...
    public static void EnlistTroopsForMission(Mission mission)
    {
        foreach (var pair in mission.factory.links.OrderBy(p => p.Key.enemyCountWeighted))
        {
                ...
                && mission.factory != ally)
            {
                ...
                if (mission.type == MissionType.Bomb && mission.troopsNeeded < distance)
                {
                    continue;
                }
    ...
    public static void SolveCaptureMission(Mission mission)
    {
        ...
        foreach (var ally in Factory.ally.Values)
        {
                ...
                && mission.factory != ally)
            {
                ...
                Factory next = BFS.UseBFS(ally, mission.factory);
                int distance = ally.links[next];
                ...
                Action newAction = new Action(MoveType.Move, ally, next, robotsAvailable);
                AddAction(newAction);
            }
        }
    }
