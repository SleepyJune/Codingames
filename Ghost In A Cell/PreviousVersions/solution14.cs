...
class BFS
{
    public static Dictionary<Factory, int> dist = new Dictionary<Factory, int>();
    public static Dictionary<Factory, Factory> prev = new Dictionary<Factory, Factory>();

    public static void UseDijkstra(Factory start, Factory end)
    {
        BFS.dist = new Dictionary<Factory, int>();
        BFS.prev = new Dictionary<Factory, Factory>();

        List<Factory> q = new List<Factory>();

        foreach (var factory in Factory.factories.Values) //initialize
        {
            BFS.dist.Add(factory, int.MaxValue);
            BFS.prev.Add(factory, null);
        ...
        while (q.Count > 0)
        {
            ...
            foreach (var pair in factory.links)
            {
                ...
                int alt = BFS.dist[factory] + distance;

                if (alt < BFS.dist[neighbour])
                {
                    BFS.dist[neighbour] = distance;
                    BFS.prev[neighbour] = factory;
                }
            }
        }
