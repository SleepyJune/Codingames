...
class Mission
{
    ...
    public Dictionary<Factory, Troop> acceptedMission;
    ...
    public Mission(MissionType type, Factory factory, int troopsNeeded)
    {
        ...
        this.acceptedMission = new Dictionary<Factory, Troop>();
...
class Game
{
    ...
    public static void SolveCaptureMission(Mission mission)
    {
        foreach (var pair in mission.factory.links.OrderBy(p => p.Value))
        {
            ...
            if (ally.isAlly && ally.count > 1 && distance < 10)
            {
                ...
                Troop mockTroop = new Troop
                {
                    ...
                    isEnemy = false,
                    turns = distance,
                    endTime = Game.gameTime + distance,
                };

                mission.acceptedMission.Add(ally, mockTroop); //ally factory accepts the mission
        ...
        var orderedAllies = mission.acceptedMission.OrderBy(p => p.Value.endTime); //ordered by distance
        ...
        Dictionary<Factory, Troop> newEnlisted = new Dictionary<Factory, Troop>();
        ...
        {
            Factory ally = pair.Key;
            Troop troop = pair.Value;

            FactoryState newState = new FactoryState(lastState, troop);
            newEnlisted.Add(ally, troop);
        ...
        if (isMissionPossible)
        {
            foreach (var pair in newEnlisted)
            {
                ...
                Action newAction = new Action(MoveType.Move, ally, mission.factory, troop.count);
                AddAction(newAction);
            }
        }
    }
    ...
    public static List<Mission> DefendAllyFactories()
    {
        ...
        {
            if (ally.oldProduction == 0 && Factory.enemyTotalProduction != 0)
            {
                continue;
            }
    ...
    public static List<Mission> CaptureEnemyFactories()
    {
        ...
        {
            if (factory.oldProduction == 0 && Factory.enemyTotalProduction != 0)
            {
                continue;
            }
