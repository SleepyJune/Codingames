...
class FactoryState
{
    ...
    public static FactoryState GetFactoryState(Factory factory, Troop testTroop)
    {
        ...
        {
            if (state.troop.id == testTroop.id)
            {
                return state;
            }
        }
    ...
    public void Solve()
    {
        ...
        foreach (var mockTestTroop in mission.acceptedMission)
        {
            ...
            if (allyLock)//mission is possible
            {
                ...
                {
                    if (!MissionReward.GetCaptureProbability(mission, mockTestTroop.count, mission.missionEndTime - Game.gameTime))
                    {
    ...
    public static bool GetCaptureProbability(Mission mission, int defendingArmy, int turns)
    {
        ...
        foreach (var pair in mission.factory.shortestPaths.shortestDistance)
        {
            ...
            if (distance <= 4)
            {
                if (neighbour.isAlly)
                {
                    defendingArmy += neighbour.count;
                }
                else if (neighbour.team == Team.Enemy)
                {
                    maxEnemyTroopCount += neighbour.count;

                    int production = neighbour.production * turns;
                    maxEnemyTroopCount += production;
                }
            }
        }
...
class Strategy
{
    ...
    public static bool ExecuteMissions()
    {
        ...
        for (int i = 0; i < missions.Count; i++)
        {
            ...
            Console.Error.WriteLine(mission.factory.id + ": " + mission.successRating.ToString());
